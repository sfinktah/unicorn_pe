...
// clang-format off
#define TRUECOLOR(r, g, b) "\033[38;2;" #r ";" #g ";" #b "m"
#define RESETFG "\033[39m"

std::string color_disasm(std::string pattern) {
    pattern = pystring::replace(pattern, "rax", TRUECOLOR(175, 175, 255) "rax" RESETFG); 
    pattern = pystring::replace(pattern, "rbx", TRUECOLOR(255, 135, 0) "rbx" RESETFG); 
    pattern = pystring::replace(pattern, "rcx", TRUECOLOR(255, 175, 0) "rcx" RESETFG); 
    pattern = pystring::replace(pattern, "rdx", TRUECOLOR(255, 255, 0) "rdx" RESETFG); 
    pattern = pystring::replace(pattern, "rbp", TRUECOLOR(175, 255, 0) "rbp" RESETFG); 
    pattern = pystring::replace(pattern, "rsp", TRUECOLOR(0, 255, 0) "rsp" RESETFG); 
    pattern = pystring::replace(pattern, "rdi", TRUECOLOR(0, 255, 175) "rdi" RESETFG); 
    pattern = pystring::replace(pattern, "rsi", TRUECOLOR(0, 255, 255) "rsi" RESETFG); 
    pattern = pystring::replace(pattern, "r8", TRUECOLOR(0, 175, 255) "r8" RESETFG); 
    pattern = pystring::replace(pattern, "r9", TRUECOLOR(175, 95, 95) "r9" RESETFG); 
    pattern = pystring::replace(pattern, "r10", TRUECOLOR(175, 135, 95) "r10" RESETFG); 
    pattern = pystring::replace(pattern, "r11", TRUECOLOR(135, 175, 95) "r11" RESETFG); 
    pattern = pystring::replace(pattern, "r12", TRUECOLOR(95, 175, 95) "r12" RESETFG); 
    pattern = pystring::replace(pattern, "r13", TRUECOLOR(95, 175, 135) "r13" RESETFG); 
    pattern = pystring::replace(pattern, "r14", TRUECOLOR(95, 135, 175) "r14" RESETFG); 
    pattern = pystring::replace(pattern, "r15", TRUECOLOR(95, 95, 175) "r15" RESETFG); 
    return pattern;
}
std::string wrap_replacement(std::string pattern) {
    return pattern;
}
std::string wrap_pattern(std::string pattern, vector_string& groupnames, group_t& capture_groups, int format) {
    ...
    pattern = pystring::replace(pattern, R"([::qword_ptr::])",                R"()");
    pattern = pystring::replace(pattern, R"([::dword_ptr::])",                R"()");
    pattern = pystring::replace(pattern, R"([::word_ptr::])",                 R"()");
    pattern = pystring::replace(pattern, R"([::byte_ptr::])",                 R"()");
    pattern = pystring::replace(pattern, R"([::rel::])",                      R"()");
	if (format == 1) {
		try {
