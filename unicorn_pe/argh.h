...
namespace argh {
    ...
    private:
        ...
        mutable std::set<std::string> used_params_;
        std::string empty_;
    };
    ...
    inline string_stream parser::operator()(std::string const& name) const {
        ...
        if (params_.end() != optIt) {
            auto stripped_name = trim_leading_dashes(name);
            used_params_.emplace(stripped_name);
    ...
    inline string_stream parser::operator()(std::initializer_list<char const* const> init_list) const {
        for (auto& name : init_list) {
            auto optIt = params_.find(trim_leading_dashes(name));
            if (params_.end() != optIt) {
                for (auto& name2 : init_list)
                    used_params_.emplace(trim_leading_dashes(name2));
            }
            return string_stream(optIt->second);
        }
        return bad_stream();
