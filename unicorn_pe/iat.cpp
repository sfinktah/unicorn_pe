...
IMAGE_OPTIONAL_HEADER* __fastcall Get_IMAGE_OPTIONAL_HEADER(IMAGE_FILE_HEADER* fheader) {
    return (IMAGE_OPTIONAL_HEADER*)&fheader[1];
}
IMAGE_FILE_HEADER* __fastcall Get_IMAGE_FILE_HEADER(IMAGE_DOS_HEADER* ImageBase) {
    return (IMAGE_FILE_HEADER*)((char*)&ImageBase->e_cp + (unsigned int)ImageBase->e_lfanew);
}

s_segment stored_section_sizes[]{
    {0x140001000, 0x1913200},       // 0
    {0x141915000, 0x0000C00},       // 1
    {0x141916000, 0x0000060},       // 2
    {0x141917000, 0x03B6600},       // 3
    {0x141CCE000, 0x11F4F28},       // 4
    {0x142EC3000, 0x0104C00},       // 5
    {0x142FC8000, 0x0000A00},       // 6
    {0x142FC9000, 0x0000200},       // 7
    {0x142FCA000, 0x002DE00},       // 8
    {0x142FF8000, 0x007A800},       // 9
    {0x143073000, 0x18C0A00},       // 10
    {(uint64_t)-1, (uint64_t)-1}};  // 11

s_segment stored_section_sizes_323_1[]{
	{0x141719000, 0x0C00},
    {0x14171A000, 0x60},
    {0x14171B000, 0x369800},
    {0x141A85000, 0x0FFA4E8},
    {0x142A80000, 0x0F1000},
    {0x142B71000, 0x0A00},
    {0x142B72000, 0x200},
    {0x142B73000, 0x2DE00},
    {0x142BA1000, 0x0D5600},
    {0x142C77000, 0x125CC00},
	{(uint64_t)-1, (uint64_t)-1}
};

int __fastcall CheckSegmentLengths(char* ImageBase, s_segment* segment_offsets) {
    ...
    fhead = Get_IMAGE_FILE_HEADER((IMAGE_DOS_HEADER*)ImageBase);
    if (!fhead)
        return 0;
    ohead = Get_IMAGE_OPTIONAL_HEADER(fhead);
    // DataDirectory[0] + Number of data directories
    shead = (IMAGE_SECTION_HEADER*)&ohead->DataDirectory[ohead->NumberOfRvaAndSizes];
    i     = 0;
    if (!fhead->NumberOfSections)
        return segment_offsets[fhead->NumberOfSections].ptr == -1i64;
    while (segment_offsets[i].ptr != -1i64 && segment_offsets[i].size == shead->Misc.VirtualSize) {
        ++shead;
        if (++i >= (unsigned int)fhead->NumberOfSections)
            return segment_offsets[fhead->NumberOfSections].ptr == -1i64;
    }
    return 0;
}
...
DWORD PeEmulation::RebuildSectionSizes(PVOID ImageBase, ULONG ImageSize) {
	...
	DWORD lastVirtualSize = 0;
	DWORD firstVirtualAddress = 0;
	DWORD finalVirtualSize = 0;
    while (i++ < fhead->NumberOfSections) {
        ...
        if (i == 0)
			firstVirtualAddress = shead->VirtualAddress;
		finalVirtualSize = lastVirtualAddress + size - firstVirtualAddress;
		lastVirtualSize = size;
		lastVirtualAddress = shead->VirtualAddress;

        ++shead;
    }

	*outs << fmt::format("{:16} TotalVirtualSize {:8x} -> {:8x}\n", "", ohead->SizeOfImage, finalVirtualSize);
	// ohead->SizeOfImage = finalVirtualSize;
    return finalVirtualSize;
}
...
bool PeEmulation::RebuildSection(PVOID ImageBase, ULONG ImageSize, virtual_buffer_t& RebuildSectionBuffer) {
    ...
    for (size_t j = 0; j < outVMPCall.size(); ++j) {
        ...
        if (call_target_rva >= 0 && call_target_rva < (int)ImageSize) {
            ...
            if (FindSectionByAddress(call_target_addr, &call_addr_section) && call_addr_section) {
                if (0 == memcmp(call_addr_section->SectionName, ".text\0\0\0", 0) || 0 == memcmp(call_addr_section->SectionName, "INIT\0\0\0\0", 0) || 0 == memcmp(call_addr_section->SectionName, "PAGE\0\0\0\0", 0)) {
    ...
    for (size_t j = 0; j < vmpcalls.size(); ++j) {
        ...
        if (err == UC_ERR_OK && m_LastFakeAPICall && (((m_LastFakeAPICallReturnAddress == call_ret || m_LastFakeAPICallReturnAddress == call_ret + 1) && tempCtx.Rsp + sizeof(uint64_t) == m_StackEnd - 0x1000) || (m_LastFakeAPICallReturnAddress == 0 && tempCtx.Rsp == m_StackEnd - 0x1000))) {
            FakeAPI_t* api_ptr = NULL;
