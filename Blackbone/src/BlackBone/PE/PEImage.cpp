...
namespace blackbone {
    ...
    namespace pe {
        ...
        NTSTATUS PEImage::Load(const std::wstring& path, bool skipActx /*= false*/, PreMapCallback preMapCallback) {
            Release(true);
            _imagePath = path;
            _noFile    = false;
            ...
            auto status = Parse(nullptr, preMapCallback);
        ...
        NTSTATUS PEImage::Reload() {
            return Load(_imagePath, false, nullptr);
        }
        ...
        /// </summary>
        /// <returns>Status code</returns>
        NTSTATUS PEImage::Parse(void* pImageBase /*= nullptr*/, PreMapCallback preMapCallback) {
            ...
            if (_pImageHdr32->OptionalHeader.Magic == IMAGE_NT_OPTIONAL_HDR64_MAGIC) {
                ...
                if (preMapCallback)
					preMapCallback(0, *this, _pImageHdr64);
            } else {
                _is64 = false;
            ...
            if (_isPureIL) {
                _ILFlagOffset = static_cast<int32_t>(
                    reinterpret_cast<uint8_t*>(pCorHdr) - reinterpret_cast<uint8_t*>(_pFileBase) + static_cast<int32_t>(offsetof(IMAGE_COR20_HEADER, Flags)));
            ...
            if (useDelayed) {
                ...
                for (; pImportTbl->Name; ++pImportTbl) {
                    ...
                    pRVA = reinterpret_cast<uint8_t*>(ResolveRVAToVA(
                        pImportTbl->OriginalFirstThunk ? pImportTbl->OriginalFirstThunk : pImportTbl->FirstThunk));
